...
void audio_board_init(void)
{
    ...
    if(!ret)
    {
        Serial.println("i2s.setDATA error");
    }
    ret = i2s.setBCLK(27); // Note: LRCLK = BCLK + 1
    if(!ret)
    {
        Serial.println("i2s.setBCLK error");
    }
    ret = i2s.setMCLK(26); // Note: LRCLK = BCLK + 1
    if(!ret)
    {
        Serial.println("i2s.setMCLK error");
    }
    ret = i2s.setBitsPerSample(16);
    if(!ret)
    {
        Serial.println("i2s.setBitsPerSample error");
    }
    ret = i2s.setFrequency(32000);
    if(!ret)
    {
        Serial.println("i2s.setFrequency error");
    }
    ...
    if(!ret)
    {
        Serial.println("i2s.begin error");
    }
